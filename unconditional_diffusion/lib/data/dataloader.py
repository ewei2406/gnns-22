...
def load_DGL(dataname: str) -> dgl.data.DGLDataset:
    """
    Available: [
        cora, citeseer, flickr, BlogCatalog, Polblogs, chameleon
    ]
    """
    if dataname == 'cora':
    ...
    if dataname in ['flickr', 'BlogCatalog', 'Polblogs']:
        return load_cached(dataname)
    if dataname == 'chameleon':
        return load_SNAP_DGL(
            edges_name="./chameleon/musae_chameleon_edges.csv",
            labels_name="./chameleon/musae_chameleon_target.csv",
            feat_name="./chameleon/musae_chameleon_features.json"
        )
...
def load_SNAP_DGL(
    ...
    graph = dgl.from_networkx(load_graph(edges_name))

    # Load labels
    labels = torch.tensor(load_labels(labels_name))

    graph.ndata['label'] = labels

    # Load feats
    rawFeat = load_features(feat_name)
    feat = torch.zeros((graph.num_nodes(), 4000))
    for key in rawFeat:
        feat[key] = idx_to_bool(torch.tensor(rawFeat[key]), 4000)
